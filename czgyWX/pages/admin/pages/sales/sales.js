...
Page({
  ...
  details: function (e) {
    ...
    wx.navigateTo({
      url: '../details-sales/details-sales?guid=' + e.currentTarget.dataset.text,
    })
  },
  /**
   * 生命周期函数--监听页面加载
  ...
  onLoad: function () {
    var self = this;
    wx.request({
      url: host + 'admin/saleVolume/getAllSalesVolumeInfo',
      header: {
  ...
  searchSales: function (e) {
    var self = this;
    self.setData({
      salesDatas: []
    });
    console.log(e + "searchInstock");
    var url = host + 'admin/saleVolume/selectAllSalesVolumeByProductVersion?productVersion=' + self.data.searchKey;
    wx.request({
      url: url,
      header: {
  ...
  addSales: function (e) {
    wx.navigateTo({
      url: '../details-sales/details-sales?guid=0'
    })
  },
  logout: function () {
    app.logout();
